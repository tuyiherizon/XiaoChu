...
public class BallInfoSPBombSmallLighting : BallInfoSPBase
{
    ...
    private List<BallInfo> GetBombBalls(List<BallInfo> moveBalls)
    {
        ...
        if (moveBall._BallInfoSP is BallInfoSPBombSmallLighting)
        {
            ...
            {
                var randomIdxs = GameRandom.GetIndependentRandoms(0, bombPreRandom.Count, 8);
        ...
        else
        {
            ...
            {
                var randomIdxs = GameRandom.GetIndependentRandoms(0, bombPreRandom.Count, 5);
                foreach (var idx in randomIdxs)
                {
                    bombBalls.Add(bombPreRandom[idx]);
                }
